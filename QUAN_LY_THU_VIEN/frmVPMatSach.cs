...
{
    public partial class frmVPMatSach : Form
    {
        public frmVPMatSach()
        {
            InitializeComponent();
        ...
        private void tbxMaDG_TextChanged(object sender, EventArgs e)
        {
            ...
            if (dr.Read())
            {
                ...
                if (dr2.Read())
                {
                    ...
                    con.Close();
                    string sql3 = ("select SOTIENPHATLAMMATSACH from quydinh where maquydinh in ( select maquydinh from nhomdocgia where tennhom = N'" + tbxTenNhom.Text + "')");
                    SqlCommand cmd3 = new SqlCommand(sql3, con);
                    con.Open();
                    SqlDataReader dr3 = cmd3.ExecuteReader();
                    if (dr3.Read())
                    {
                        tbxMatSach.Text = dr3[0].ToString();
                        con.Close();
                    }
                    else
                    {
                        tbxMatSach.Text = null;
                        con.Close();
                    }
                }
            }
        ...
        private void tbxMatSach_TextChanged(object sender, EventArgs e)
        {
            if (tbxMatSach.Text == null || tbxGiaBia.Text == null || tbxMatSach.Text == "" || tbxGiaBia.Text == "")
            {
            }
            else
            {
                int tien = Convert.ToInt32(tbxMatSach.Text) + Convert.ToInt32(tbxGiaBia.Text);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string b = reader.GetValue(0).ToString();
                    SqlCommand cmd1 = new SqlCommand("update thongtinnopphi set madocgia = '" + tbxMaDG.Text + "', ngaynop = '" + tbxNNop.Text + "', phatmatsach = '" + Convert.ToInt32(tbxTien.Text) + "' where maphinop = '" + b + "'", Con);
                    Con.Close();
                    SqlDataAdapter da = new SqlDataAdapter(cmd1);
                    DataSet ds = new DataSet();
                    da.Fill(ds);
                    MessageBox.Show("Nộp phí mất sách thành công");
                    tbxMaDG.Text = null;
                    ...
                    tbxTien.Text = null;
                    tbxMatSach.Text = null;
                    frmCRPhatMatSach mkk = new frmCRPhatMatSach();
                    mkk.Show();
        ...
        private void frmVPMatSach_Load(object sender, EventArgs e)
        {
